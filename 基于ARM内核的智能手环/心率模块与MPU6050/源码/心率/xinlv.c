...
{	
		...
		{		
				...
				//对BPM值进行范围划分
				if(BPM>50&&BPM<100)
					LCD_ShowStringAndHz(60,210,40,16,16,"Good!",0);	
				else	
					LCD_ShowStringAndHz(60,210,40,16,16,"Bad! ",0);
		}
}

void Show_SensorView(void)
{    
	LCD_ShowStringAndHz(60,150,56,16,16,"* BPM: ",0);
	LCD_ShowStringAndHz(60,170,56,16,16,"* IBI",0);
...
void Show_Sensor(u16 signal)
{
	...
	if(rate_x<240)
	{
			...
			POINT_COLOR = WHITE;
			LCD_DrawLine(rate_x,10,rate_x,130);//清除上一次线段
	}
	...
	rate_y_buf=rate_y;
	POINT_COLOR = RED;
	LCD_DrawLine((rate_x-1),rate_y_old,(rate_x-1),rate_y_buf);
  /*绘制心电图结束，保存刚接收到的心率信号*/			
