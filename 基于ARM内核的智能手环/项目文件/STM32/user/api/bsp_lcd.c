...
//显示字符串
//x,y:起点坐标
//width,height:区域大小(给定width*height区域用于显示)
//size:字体大小  12/16/24
//*p:字符串起始地址
void LCD_ShowString(uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint8_t size,uint8_t *p,u16 color)
{
  uint8_t x0=x;
  width+=x;
  height+=y;
	...
	{                              
		for(i=0;i<width/8;i++)//列
		{
				for(k=0;k<8;k++)
				{
					...
					LCD_Fast_DrawPoint(x,y,f_color);
					//	TFTLCD->LCD_RAM=f_color;//描字符颜色
					else
						//TFTLCD->LCD_RAM=b_color;//描背景颜色 
					LCD_Fast_DrawPoint(x,y,b_color);
...
void LCD_ShowStringAndHz(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p,u8 mode,u16 color)
{
	...
	while (*p != '\0')
	{
		if (*p > 0xa0)				//汉字内码都是大于0xa0
		{
			if(x>=width){x=x0;y+=size;}
			if(y>=height)break;//退出
			if(size == 12)
			{
				for(j=0;j<hz12_num;j++)
				{
					if((Hz12X12[j].Index[0] == *p) && (Hz12X12[j].Index[1] == *(p+1)))
					{
						for(i=0;i<size;i++)
						{
							c = (Hz12X12[j].Msk[2*i]<<8) | Hz12X12[j].Msk[2*i+1];
							for(k=0;k<size;k++)
							{
								if((c<<k) & 0x8000)
									LCD_Fast_DrawPoint(x+k,y+i,color);
								else if(mode==0)
									LCD_Fast_DrawPoint(x+k,y+i,BACK_COLOR);
							}
						}
						break;
					}
			...
			{
				for(j=0;j<hz16_num;j++)
				{
					if((Hz16X16[j].Index[0] == *p) && (Hz16X16[j].Index[1] == *(p+1)))
					{
						for(i=0;i<size;i++)
						{
							c = (Hz16X16[j].Msk[2*i]<<8) | Hz16X16[j].Msk[2*i+1];
							for(k=0;k<size;k++)
							{
								if((c<<k) & 0x8000)
									LCD_Fast_DrawPoint(x+k,y+i,color);
			...
			{
				for(j=0;j<hz24_num;j++)
				{
					if((Hz24X24[j].Index[0] == *p) && (Hz24X24[j].Index[1] == *(p+1)))
					{
						for(i=0;i<size;i++)
						{
							c = (Hz24X24[j].Msk[3*i]<<16) | (Hz24X24[j].Msk[3*i+1]<<8) | Hz24X24[j].Msk[3*i+2];
							for(k=0;k<size;k++)
							{
								if((c<<k) & 0x800000)
									LCD_Fast_DrawPoint(x+k,y+i,color);
		...
		{	
			if(x>=width){x=x0;y+=size;}
			if(y>=height)break;//退出
			LCD_ShowChar(x,y,BACK_COLOR,color,*p,size,mode);
			x+=size/2;
			p++;
		}
	}
